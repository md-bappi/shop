...
const SliderHero: React.FC = () => {
  ...
  const sliderImages =
    products && products.length > 0
      ? products.map((p: any) => p.image?.url || "")
      : [
  ...
  return (
    <section className="w-full h-[calc(100vh-4rem)] bg-gradient-to-r from-sky-500 to-indigo-600">
      <div className="max-w-7xl mx-auto h-full px-6">
        <div className="grid grid-cols-6 h-full gap-6 items-center">
          {/* Left: text area (4 cols) */}
          ...
          {/* Right: static image area (2 cols) */}
          <div className="col-span-6 md:col-span-2 relative">
            <div className="relative w-full h-full rounded-2xl overflow-hidden border border-white/10 bg-white/5 flex items-center justify-center">
              ...
              <button
                onClick={handlePrev}
                className="absolute left-2 top-1/2 -translate-y-1/2 bg-gradient-to-r from-sky-500 to-indigo-500 hover:opacity-90 text-white rounded-full p-2 shadow-lg backdrop-blur-sm border border-white/20 transition cursor-pointer"
              >
                ‹
              </button>
              ...
              <button
                onClick={handleNext}
                className="absolute right-2 top-1/2 -translate-y-1/2 bg-gradient-to-r from-indigo-500 to-sky-500 hover:opacity-90 text-white rounded-full p-2 shadow-lg backdrop-blur-sm border border-white/20 transition cursor-pointer"
              >
                ›
              </button>
            </div>
